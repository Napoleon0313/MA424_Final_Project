...
# variables
K = 3 # 3 vehicles
# variables encoding the which vehicle serves which clients
y = m.addVars(n+1,K,vtype = 'B',name = 'y') 
...
# degree constraints
m.addConstrs((quicksum(y[i,k] for k in range(K)) == 1 
	for i in range(1,n+1)),'serve1')
m.addConstr((quicksum(y[0,k] for k in range(K)) == K),'serve2')
# 1.31(1)
...
m.addConstrs((u[i,k]-u[j,k]+routing_dem[j] <= sum(routing_dem)*(1-x[i,j,k]) for i in range(n+1) 
...
results.to_excel("D:/Data File/Graduate-LSE/MA424 Modelling in Operations Research/Final Project/2.1 Multiple Vehicle Routing_result.xlsx")
